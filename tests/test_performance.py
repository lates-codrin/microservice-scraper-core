...
def test_health_endpoint_p95_slo(client):
    ...
    latencies = []
    headers = {"Authorization": "Bearer dev-api-key-change-me", "X-Request-ID": "perf-test", "X-Tenant-ID": "perf-tenant"}
    
    for i in range(100):
        start = time.monotonic()
        response = client.get("/v1/health", headers={**headers, "X-Request-ID": str(uuid.uuid4()), "X-Tenant-ID": f"perf-tenant-{i}"})
...
def test_classify_endpoint_p95_slo(client):
    ...
    for i in range(50):
        ...
        response = client.post(
            "/v1/classify",
            json=payload,
            headers={**headers, "X-Request-ID": str(uuid.uuid4()), "X-Tenant-ID": f"perf-tenant-{i}"},
        )
        elapsed = (time.monotonic() - start) * 1000  # ms
        latencies.append(elapsed)
        assert response.status_code == 200
...
def test_metrics_endpoint_p95_slo(client):
    ...
    for i in range(100):
        start = time.monotonic()
        response = client.get("/v1/metrics", headers={**headers, "X-Request-ID": str(uuid.uuid4()), "X-Tenant-ID": f"perf-tenant-{i}"})
        elapsed = (time.monotonic() - start) * 1000  # ms
        latencies.append(elapsed)
        assert response.status_code == 200
...
def test_concurrent_requests_throughput(client):
    ...
    def make_request(i):
        ...
        response = client.get(
            "/v1/health",
                headers={"Authorization": "Bearer dev-api-key-change-me", "X-Request-ID": str(uuid.uuid4()), "X-Tenant-ID": f"tenant-{i}"},
        )
        elapsed = (time.monotonic() - start) * 1000
        return elapsed, response.status_code
    
    # Warm-up request to pay the ~2s NullPool connection cost upfront
    client.get("/v1/health", headers={"Authorization": "Bearer dev-api-key-change-me", "X-Request-ID": "warmup", "X-Tenant-ID": "tenant-warmup"})

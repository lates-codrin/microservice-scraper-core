...
def run_baseline():
    ...
    cmd = [
        ...
        "--headless", "-u", "10", "-r", "10",
        "--run-time", "60s", "--host", "http://127.0.0.1:8081",
        "--csv", "tests/load/locust_stats"
    ]
    subprocess.run(cmd)
    ...
    import csv
    baseline = {}
    with open("tests/load/locust_stats_stats.csv", "r") as f:
        ...
        for row in reader:
            ...
            if "/v1/jobs/{job_id}" == name and "documents" not in name:

...
try:
    ...
    print(f"[v0] Security schemes: {list(spec.get('components', {}).get('securitySchemes', {}).keys())}")
    
    # Check for code samples on key endpoints
    scrape_samples = spec.get('paths', {}).get('/v1/scrape', {}).get('post', {}).get('requestBody', {}).get('content', {}).get('application/json', {}).get('x-codeSamples')
    crawl_samples = spec.get('paths', {}).get('/v1/crawl', {}).get('post', {}).get('requestBody', {}).get('content', {}).get('application/json', {}).get('x-codeSamples')
